package dev.idhoorz.gpclaimblocks.managers;
...
public final class RankManager {
    ...
    public void reload() {
        ...
        for (String perm : section.getKeys(false)) {
            ranks.put(perm, Math.max(0, section.getInt(perm)));
        }
    }
    ...
    private int calcAccumulative(Player player) {
        ...
        for (Map.Entry<String, Integer> e : ranks.entrySet()) {
            if (player.hasPermission(e.getKey())) total += e.getValue();
        }
        return total;
    }
    ...
    public RankInfo getRank(Player player) {
        for (Map.Entry<String, Integer> e : ranks.entrySet()) {
            if (player.hasPermission(e.getKey())) {
                return new RankInfo(e.getKey(), e.getValue());
            }
        }
    ...
    public static final class RankInfo {
        ...
        public String getName() {
            ...
            return i >= 0 ? permission.substring(i + 1) : permission;
        }
    }
}
